...
// Reusing the same contacts data
const contacts = [
  {
    id: "c1",
    name: "Rahul Sharma",
...
const ContactItem = ({ contact, onRequest }) => (
...
const RequestMoney = () => {
  ...
  const handleRequest = (contact) => {
    if (!amount) {
      setRequestStatus("Enter an amount to request");
      return;
    }
    setRequestStatus(`Requesting ₹${amount} from ${contact.name}...`);
    setTimeout(() => {
      setRequestStatus(`Request sent to ${contact.name} successfully!`);
    }, 1500);
  };
    ...
    <Layout>
      <div className="space-y-6 py-20">
        ...
        <div className="glass rounded-xl p-4 animate-scale-in">
          <h3 className="font-medium mb-3">Request Money</h3>
          <Input
          ...
          />
          <Button
            onClick={() => handleRequest({ name: "Manual Entry" })}
        ...
        <Tabs defaultValue="all" className="mt-6">
          <TabsList className="grid grid-cols-3 mb-4 rounded-full bg-[#66d6fe]">
